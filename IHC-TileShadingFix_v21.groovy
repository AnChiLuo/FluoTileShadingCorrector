// Macro Name: IHC-TileShadingFix
// Version: v21.0
// Author: An-Chi, Luo
// Date: [2025-06-05]
// Note: use BigStitcher to stitch Tiles and build a xml creator (Clean code)
//
// Description:
// 1. Load a multi-tile CZI image via Bio-formats without auto-stitching, splitting it into individual tile stacks.
...
//
//Notes:
//  • Only unstitched CZI images are supported!!
...
def channelData = [:]
(1..nChannels).each { c ->
    channelData ["Raw_C${c}.tiff"] = [
            label : meta.getChannelName(1, c-1),
...
if (absoluteF.size() >1 && absoluteF.size() == nTiles){
    ...
    tilePosition.eachWithIndex{ p,i ->
        tilePosition[i][0] = Math.round(p[0] / pixelScaleX *10)/10
        tilePosition[i][1] = Math.round(p[1] / pixelScaleY *10)/10
    }
}
...
def Indx1 = ["Raw_C${ (int)refChan }.tiff"]
basicImps = Indx1 + (basicImps - Indx1)
/// -- Run Basic -- ///
for ( int i = 0; i < nChannel; i++ ){
    def f = new File (basicTemp, basicImps[i])
    def cf = new File (basicTemp, "Corrected_${basicImps[i]}")
...
/// -- Save Basic result as split image with series name -- ///
    def tempstack = CorrectedImg.getStack()
    ...
    WindowManager.getImage("Corrected_${basicImps[i]}")?.close()
...
/// -- Perform pairwise shift calculation for Stitcher and discard links exceeding the shift threshold -- ///
    if ( i == 0 ) {
        ...
        IJ.run("Filter pairwise shifts ...", "select=${xmlPath.toString()} min_r=0 max_r=1 filter_by_shift_in_each_dimension max_shift_in_x=${x_shift} max_shift_in_y=${y_shift} max_shift_in_z=0 max_displacement=0");
...
/// -- Fusion tiles -- ///
    def fff = new File (Result, "Fused_${channelData[ basicImps[i]].label}.tiff")
    ...
    Fused.setTitle("Fused_${channelData[ basicImps[i]].label}")
    IJ.run("${bitDepth}-bit")
    //new WaitForUserDialog("Check").show()
    def iter = [value: i]
    if(mode == "Manual"){
        ...
        SwingUtilities.invokeLater {
            ...
            def frame = new JFrame("Check the Result of ${channelData[ basicImps[i]].label}")
